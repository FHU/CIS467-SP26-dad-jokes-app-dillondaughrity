...
export default function App() {
  ...
  const [view, setView] = useState<"random" | "browse" | "add" | "daily">("random");
  ...
  return (
    <div style={styles.page}>
      ...
      <nav style={styles.nav}>
        {(["random", "daily", "browse", "add"] as const).map((v) => (
          <button
            key={v}
            onClick={() => setView(v)}
            style={{
        ...
        {view === "daily" && (
          <div style={styles.card}>
            <h2 style={{ color: "#facc15", marginBottom: "1rem" }}>
              🌞 Joke of the Day ({new Date().toLocaleDateString()})
            </h2>
            ...
            {joke ? (
              ...
              <p style={{ color: "#aaa" }}>Loading today’s joke…</p>
            )}

            <p style={{ marginTop: "1rem", fontSize: "0.8rem", color: "#94a3b8" }}>
              Same joke all day — come back tomorrow for a new one!
            </p>
          </div>
        )}
