...
// --- GET /api/jokes/today – joke of the day ----------------------------
app.get("/api/jokes/today", async (req: Request, res: Response) => {
  try {
    ...
    const jokeResult = await pool.query(
      "SELECT * FROM jokes OFFSET $1 LIMIT 1",
      [offset]
    );

    res.json(jokeResult.rows[0]);
  } catch (err) {
    res.status(500).json({ error: "Failed to fetch joke of the day" });
  }
});
